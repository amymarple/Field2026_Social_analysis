...
def read_episode_history(episode_id: str) -> list[dict]:
    """Read append-only standard annotation records for one episode across sessions."""
    rows: list[dict] = []
    if not ANNOT_DIR.exists():
        return rows
    for path in sorted(ANNOT_DIR.glob("annotations_*.jsonl")):
        for record in read_log(path):
            if str(record.get("episode_id")) == str(episode_id):
                rows.append(record)
    return sorted(rows, key=lambda record: str(record.get("ts", "")))
