...
It reads (never writes) the canonical files and adapts them into small plain dicts:
  ...
  * field geometry   <- cv/configs/field_layout.json  (cm, origin A0)
  * zone polygons    <- wiser/configs/wiser_rois.json           (WISER inches)
  * subject identities <- wiser/configs/rat_identities.csv      (shortid -> name)
...
WISER_ROIS = REPO_ROOT / "wiser" / "configs" / "wiser_rois.json"
RAT_IDENTITIES = REPO_ROOT / "wiser" / "configs" / "rat_identities.csv"
