...
Add a pure, testable helper in `audio/src/audio_io.py` and call it from
...
- `audio/src/audio_io.py` — new `_dedup_overlapping`; call it in `find_recording_files`.
- `audio/scripts/selftest_features.py` (or a new `selftest_discovery.py`) — offline test
...
- Re-extraction: hour 14 returns to ~60 windows/channel; the previously double-counted silent
  ...
  removed duplicates.
- Raw MP4s untouched; outputs only under git-ignored `audio/outputs/`.
