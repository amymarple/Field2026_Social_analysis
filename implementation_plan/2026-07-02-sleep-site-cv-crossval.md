...
independent CV shelter-occupancy signal (`cv/shelter_sleep.py`): when
...
- CV `cv/outputs/CH05_sleep_{2026-06-29,2026-06-30}.csv` (left shelter)
...
- Shelter ROIs `house_1`,`house_2` in `wiser/configs/wiser_rois.json` (placed).
...
(occupancy overlay + kappa-vs-lag); manifest + verdict → `D:\Field2026_analysis_out\sleep_site_cv_crossval_*`.
...
- Outputs only to `D:\Field2026_analysis_out`; no writes to source DBs or CV outputs.
