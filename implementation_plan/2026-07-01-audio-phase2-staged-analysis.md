...
(`audio/outputs/audio_features_CHxx_<date>.csv`). The scientific value comes from the
...
- `audio/src/audio_io.py` — `iter_windows` / `_stream_pcm` for spectrogram re-decode.
- `audio/src/plotting.py` — `plot_level_over_time`, `plot_index_timeseries`,
  `plot_window_spectrogram`.
- `wiser/src/wiser_io.py` — `load_wiser_sqlite`, `load_sqlite_window`,
  `_standardise_df` (for the gz incrementals).
- `wiser/src/time_utils.py`, `metrics.py`, `wiser_analysis_utils.py` — WISER
...
|---|---|---|
| Audio features | `audio/outputs/audio_features_CHxx_<date>.csv` | `window_start_timestamp` = local wallclock (recorder filename time); filter `valid_audio` |
