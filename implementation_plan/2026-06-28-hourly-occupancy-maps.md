...
`wiser/` can analyse a finished session folder, but has no hour-resolved spatial
...
- Update `wiser/src/wiser_io.py` — read-only SQLite open + `load_sqlite_window`.
- Update `wiser/src/plotting.py` — add `plot_occupancy_grid`.
- Add `wiser/scripts/plot_hourly_occupancy.py`.
- Add `wiser/install_wiser_occupancy_task.ps1` (hourly scheduled task).
- Add cached extent `wiser/configs/arena_extent.json` (generated at runtime).
...
Outputs:
- `wiser/outputs/occupancy/occupancy_<tz>_YYYY-MM-DD_HH.png`, one per completed
