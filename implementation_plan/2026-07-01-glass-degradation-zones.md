...
- new `cv/view_quality.py` — GPU-free. Per-zone view_quality in 3 tiers
...
- new `cv/place_zones.py` — OpenCV editor to draw `inside_shelter` /
...
- new `configs/view_quality.yaml` — thresholds (sat/dark ratios, sharpness, contrast, motion, blob
  sizes), tunable.
- modify `cv/shelter_sleep.py` — zone- + view-aware rewrite of
...
- modify `cv/validate_shelter.py` — record per-sample view_quality; report
