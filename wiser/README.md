...
## Where analysis outputs go (one rule)

Every analysis driver follows one convention, centralized in **`src/output_paths.py`**:

- **Bulk run artifacts** (CSVs, figures, `run_manifest.json`) go OFF the C: drive and OFF git,
  under **`D:\Field2026_analysis_out\<name>_<YYYYMMDD_HHMM>\`** — a fresh timestamped folder per run.
- **The repo keeps only the report** at `outputs/<direction>/<...>_report.md`, plus a
  `LATEST_RUN.txt` pointer naming the D: run folder it summarizes. `outputs/` is git-ignored.

Do not hard-code `D:\Field2026_analysis_out`. Get the root from the helper so it is defined once and can be
redirected with the `FIELD2026_ANALYSIS_OUT_ROOT` env var (e.g. a machine without a D: drive, or a self-test):

```python
from output_paths import OUT_ROOT, run_dir, report_dir, write_latest_pointer
out = run_dir("my_analysis")            # D:\Field2026_analysis_out\my_analysis_<ts>\ (+ figures\)
rep = report_dir("direction3_my_thing") # wiser\outputs\direction3_my_thing\
write_latest_pointer(rep, out)          # rep\LATEST_RUN.txt -> out
```

Housekeeping — timestamped runs accumulate, so trim old ones (dry-run by default):

```bash
python scripts/prune_wiser_runs.py                 # show what would be deleted (keep newest 3)
python scripts/prune_wiser_runs.py --keep 5 --apply # actually delete, keeping newest 5 per analysis
python scripts/selftest_output_paths.py            # offline PASS/FAIL for the helper
```

> **Known exception (staged):** the route/policy/following/locomotor pipelines still keep their
> intermediate store under `outputs/<name>_<daterange>/` because their stages read each other's
> CSVs via in-repo paths; relocating them to `D:\Field2026_analysis_out` needs a shared per-pipeline run dir
> and is tracked as a follow-up (see `implementation_plan/2026-07-12-output-path-consolidation.md`).
...
#    wiser\configs\wiser_georef_survey.csv
