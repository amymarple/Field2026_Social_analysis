...
- The script **refuses** any `--output` equal to or under the source DB tree `D:\Wiser`
  (a sibling like `D:\Field2026_analysis_out` is fine).
- Nothing is ever written to `D:\Wiser`. Outputs default to **`D:\Field2026_analysis_out`** — off the C: drive
...
(default `D:\Field2026_analysis_out`), `--tz local|utc`, `--style scatter|occupancy`, `--bin-inches`
...
Output files: `D:\Field2026_analysis_out\<style>_<tz>_YYYY-MM-DD_HH.png`
