...
# cv/field_coords.py FIELD_X_CM / FIELD_Y_CM. Kept as
...
def circadian_rest_profile(df: pd.DataFrame, *, rest_thr_inps: float,
                           tz_offset_hours: int = LOCAL_TZ_OFFSET_HOURS,
                           valid_only: bool = True) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Per-(tag, local clock-hour) REST fraction over the full 24 h across all days present
    — the diel/circadian rest–activity profile ("when do the rats stop moving").

    REST = smoothed locomotion speed < ``rest_thr_inps`` (the stationary p99 speed-noise
    floor from :func:`speed_noise_floor`), i.e. **indistinguishable from stationary given
    jitter**. It is a low-speed **PROXY for sleep, NOT ephys-validated**, and NaN speed is
    treated as *not* resting (same rule as :func:`rest_mask`). A signal **GAP is unknown,
    never 'rest'** — so ``cover_frac`` reports how much of each clock-hour was actually
    observed; read low-coverage hours (e.g. the refuge_4 burrow at night on 07-03→07-07,
    or wet-night dropout) as lower-confidence, since dropout biases an observed-fix rest
    fraction.

    Requires ``datetime`` (naive UTC), ``speed_inps_smooth`` (:func:`add_speed`) and
    ``shortid``; a ``valid`` column is honored when ``valid_only``. Local clock-hour uses
    ``tz_offset_hours`` (EDT = UTC−4).

    Returns ``(per_tag, group)``:
      * ``per_tag``: ``shortid, clock_hour, rest_frac, n_fix, n_days, cover_frac`` —
        ``rest_frac`` = mean(resting) over that tag's valid fixes in that local hour,
        pooled over days; ``cover_frac`` = observed minutes / (n_days·60), capped at 1.
      * ``group``: ``clock_hour, rest_frac_mean, rest_frac_sem, rest_frac_sd, n_animals,
        cover_frac_mean`` — mean ± SEM taken **across tags** (SEM = SD/√n_tags, sample SD).
    """
    d = df.dropna(subset=["datetime"]).copy()
    if valid_only and "valid" in d.columns:
        d = d[d["valid"]]
    if d.get("speed_inps_smooth") is None:
        raise KeyError("Run add_speed() before circadian_rest_profile().")
    d = _add_local_time(d, tz_offset_hours)
    d["resting"] = d["speed_inps_smooth"].lt(rest_thr_inps).fillna(False)
    d["night"] = d["datetime_local"].dt.date.astype(str)
    d["min_floor"] = d["datetime_local"].dt.floor("min")
    per_tag = (d.groupby(["shortid", "clock_hour"])
               .agg(rest_frac=("resting", "mean"), n_fix=("resting", "size"),
                    n_days=("night", "nunique"), n_min=("min_floor", "nunique"))
               .reset_index())
    per_tag["cover_frac"] = (per_tag["n_min"] / (per_tag["n_days"] * 60.0)).clip(upper=1.0)
    per_tag = per_tag.drop(columns=["n_min"])
    grp = (per_tag.groupby("clock_hour")
           .agg(rest_frac_mean=("rest_frac", "mean"), rest_frac_sd=("rest_frac", "std"),
                n_animals=("rest_frac", "size"), cover_frac_mean=("cover_frac", "mean"))
           .reset_index())
    grp["rest_frac_sem"] = grp["rest_frac_sd"] / np.sqrt(grp["n_animals"].clip(lower=1))
    return per_tag, grp


def circadian_rest_by_night(df: pd.DataFrame, *, rest_thr_inps: float,
                            tz_offset_hours: int = LOCAL_TZ_OFFSET_HOURS,
                            valid_only: bool = True, anchor_hour: int = 0) -> pd.DataFrame:
    """
    **Day-resolved** circadian rest fraction: per (tag, day, clock-hour), for testing
    whether the diel rhythm DRIFTS across days (the pooled :func:`circadian_rest_profile`
    averages the drift away). Same REST proxy and the same dropout guard as that function;
    ``cover_frac`` uses a 60-minute denominator (a single day-hour). A low-coverage cell is
    missing data, NOT rest — mask on ``cover_frac`` before plotting/averaging.

    ``anchor_hour`` sets where each "day" begins in local time. **0 → calendar date** (the
    default). Set it to the daily **activity trough** (e.g. ~07:00) to get a
    **BIOLOGICAL-NIGHT** cut, so an animal's dusk→dawn active bout stays in ONE unit
    instead of being split across two calendar dates (a fix's day = local time minus
    ``anchor_hour``). The ``night`` label is then the date of that biological night's
    anchor day (i.e. the evening it began).

    Requires ``datetime`` (naive UTC), ``speed_inps_smooth`` (:func:`add_speed`),
    ``shortid``; ``valid`` honored when ``valid_only``. Columns:
    ``shortid, night, clock_hour, rest_frac, n_fix, cover_frac``.
    """
    d = df.dropna(subset=["datetime"]).copy()
    if valid_only and "valid" in d.columns:
        d = d[d["valid"]]
    if d.get("speed_inps_smooth") is None:
        raise KeyError("Run add_speed() before circadian_rest_by_night().")
    d = _add_local_time(d, tz_offset_hours)
    d["resting"] = d["speed_inps_smooth"].lt(rest_thr_inps).fillna(False)
    d["night"] = (d["datetime_local"] - pd.Timedelta(hours=int(anchor_hour))).dt.date.astype(str)
    d["min_floor"] = d["datetime_local"].dt.floor("min")
    per_cell = (d.groupby(["shortid", "night", "clock_hour"])
                .agg(rest_frac=("resting", "mean"), n_fix=("resting", "size"),
                     n_min=("min_floor", "nunique"))
                .reset_index())
    per_cell["cover_frac"] = (per_cell["n_min"] / 60.0).clip(upper=1.0)
    return per_cell.drop(columns=["n_min"])
...
# ---------------------------------------------------------------------------
# Direction 3 — evening (baseline) vs morning (weather-driven) sleep windows
# ---------------------------------------------------------------------------
# The two rest episodes that bracket the nocturnal active period have different
# drivers and are analyzed SEPARATELY:
#   * EVENING sleep (~17:00 -> the temperature-calibrated SLEEP-PERIOD end): settle
#     after the daytime heat has passed; the LEAST daytime-influenced readout of a
#     rat's baseline sleep-site preference. Rats emerge to activity as it cools, so
#     the sleep-period END is per-day temperature-calibrated (later on hot nights).
#   * MORNING sleep (~05:00-10:00, before the ~10:00 site switch): bed-down after the
#     active night; its SITE is expected to depend on temperature + (overnight) rain.
# The nocturnal rat's "day" is one full SLEEP PERIOD, NOT a 0-24 h calendar day, so
# the sleep-period end hour is measured as HOURS SINCE MIDNIGHT and may exceed 24
# (past midnight) on hot nights -- it must NOT be clamped to the calendar day.
# Frame is the UNVERIFIED inch offset -> ROI-identity + RELATIVE displacement only;
# a signal gap is 'unknown', never 'moved'; weather acts on BOTH the animal path
# and the UWB-dropout path, so results are "temperature/weather-linked", never causal.

def temperature_calibrated_sleep_end(wx: pd.DataFrame, sleep_days, *,
                                     peak_lo: int = 12, peak_hi: int = 18,
                                     floor_h: float = 18.0, ceil_h: float = 30.0,
                                     ref_lo: int = 20, ref_hi: int = 22,
                                     threshold_c: float | None = None):
    """
    Per-day, temperature-calibrated **END OF THE DAYTIME SLEEP PERIOD** (evening /
    overnight emergence to activity).

    The nocturnal rat's "day" is one full SLEEP PERIOD, **not** a 0-24 h calendar day,
    so ``sleep_end_hour`` is measured as **hours since midnight of the sleep-day** and
    **MAY EXCEED 24** (past midnight) on hot nights when the air stays warm and
    emergence is delayed -- it is **searched through the night and NOT clamped to 24**
    (the earlier "evening_end" saturated at 24 exactly because it stopped at midnight).

    Rats emerge as the evening/overnight air cools, so given one cross-day thermal
    criterion ``theta*`` a hotter night reaches it later (later ``sleep_end``). For
    each sleep-day D:

    - **afternoon peak** = time/temp of max outside-air T over local ``[peak_lo, peak_hi)``;
    - ``theta*`` = ``threshold_c`` or the **pooled median** of outside T over
      ``[ref_lo, ref_hi)`` local h across all ``sleep_days``;
    - ``sleep_end`` = first time **after the peak, searching THROUGH the night up to
      ``ceil_h`` h past midnight of D**, with outside T ``<= theta*``, expressed as
      hours since midnight D; clamped to ``[floor_h, ceil_h]``; if never -> ``ceil_h``
      (``crossed=False``, i.e. the air never cooled to theta* all night).

    Uses outside-air temperature only (frame-independent). Returns
    ``(per_day_df, theta_star)``; columns: ``sleep_day, peak_local, peak_temp_c,
    threshold_c, sleep_end_local, sleep_end_hour, crossed, n_wx``.
    """
    cols = ["sleep_day", "peak_local", "peak_temp_c", "threshold_c",
            "sleep_end_local", "sleep_end_hour", "crossed", "n_wx"]
    sleep_days = [str(n) for n in sleep_days]
    if wx is None or wx.empty or "temp_c" not in wx.columns or "datetime_local" not in wx.columns:
        rows = [{"sleep_day": n, "peak_local": pd.NaT, "peak_temp_c": np.nan,
                 "threshold_c": np.nan, "sleep_end_local": pd.NaT,
                 "sleep_end_hour": float(ceil_h), "crossed": False, "n_wx": 0}
                for n in sleep_days]
        return pd.DataFrame(rows, columns=cols), float("nan")
    d = wx.dropna(subset=["datetime_local", "temp_c"]).copy().sort_values("datetime_local")
    d["date"] = d["datetime_local"].dt.date.astype(str)
    d["hf"] = d["datetime_local"].dt.hour + d["datetime_local"].dt.minute / 60.0
    if threshold_c is None:
        ref = d[(d["hf"] >= ref_lo) & (d["hf"] < ref_hi)]
        theta = float(np.nanmedian(ref["temp_c"])) if not ref.empty else float(np.nanmedian(d["temp_c"]))
    else:
        theta = float(threshold_c)
    rows = []
    for n in sleep_days:
        midnight = pd.Timestamp(n)                      # local midnight of sleep-day D
        day = d[d["date"] == n]
        if day.empty:
            rows.append({"sleep_day": n, "peak_local": pd.NaT, "peak_temp_c": np.nan,
                         "threshold_c": theta, "sleep_end_local": pd.NaT,
                         "sleep_end_hour": float(ceil_h), "crossed": False, "n_wx": 0})
            continue
        pk = day[(day["hf"] >= peak_lo) & (day["hf"] < peak_hi)]
        peak_row = (pk.loc[pk["temp_c"].idxmax()] if not pk.empty
                    else day.loc[day["temp_c"].idxmax()])
        peak_local = peak_row["datetime_local"]
        hi_time = midnight + pd.Timedelta(hours=ceil_h)   # search THROUGH the night, past midnight
        after = d[(d["datetime_local"] > peak_local) & (d["datetime_local"] <= hi_time)]
        cross = after[after["temp_c"] <= theta]
        if not cross.empty:
            end_local = cross.iloc[0]["datetime_local"]
            raw_h = (end_local - midnight) / pd.Timedelta(hours=1)   # hours since midnight D (may be >24)
            crossed = True
        else:
            end_local = pd.NaT
            raw_h = float(ceil_h)
            crossed = False
        end_h = float(min(max(raw_h, floor_h), ceil_h))
        rows.append({"sleep_day": n, "peak_local": peak_local,
                     "peak_temp_c": float(peak_row["temp_c"]), "threshold_c": theta,
                     "sleep_end_local": end_local, "sleep_end_hour": end_h,
                     "crossed": bool(crossed), "n_wx": int(len(day))})
    return pd.DataFrame(rows, columns=cols), theta


def window_sleep_site(win_slice: pd.DataFrame, roi_cfg: dict | None = None, *,
                      window_label: str, shelters=DAYTIME_SHELTERS,
                      near_shelter_in: float = 48.0) -> pd.DataFrame:
    """
    Per **(night, shortid)** daytime rest SITE for ONE pre-sliced clock window.

    Generalizes :func:`within_day_sequence` to a single arbitrary window (the driver
    pre-filters ``win_slice`` to the window's local clock range and per-day evening
    end). Uses **resting** fixes only; frame-safe (inch offset -> ROI-identity +
    relative geometry only). One row per (night, shortid):

    ``night, shortid, window(=window_label), n_rest_fix, centroid_x, centroid_y,
    spread_in, dominant_roi, dominant_zone_class, nearest_shelter,
    dist_nearest_shelter_in, near_shelter, in_shelter_frac``.

    - ``centroid`` = median resting (x, y); ``spread_in`` = median radial distance to
      that centroid (a compactness / confidence proxy).
    - ``nearest_shelter`` / ``dist_nearest_shelter_in`` = closest named shelter ROI
      centre to the centroid; ``near_shelter`` = distance ``<= near_shelter_in``.
    - ``in_shelter_frac`` = share of resting fixes whose assigned ROI is a named
      shelter. Empty input -> empty frame with the columns.
    """
    cols = ["night", "shortid", "window", "n_rest_fix", "centroid_x", "centroid_y",
            "spread_in", "dominant_roi", "dominant_zone_class", "nearest_shelter",
            "dist_nearest_shelter_in", "near_shelter", "in_shelter_frac"]
    d = win_slice[win_slice["resting"]] if "resting" in win_slice.columns else win_slice
    d = d.dropna(subset=["x", "y"]).copy()
    if d.empty:
    ...
    for (night, sid), g in d.groupby(["night", "shortid"]):
        cx, cy = float(g["x"].median()), float(g["y"].median())
        spread = float(np.median(np.hypot(g["x"] - cx, g["y"] - cy)))
        rc = g["roi"].value_counts() if "roi" in g.columns else pd.Series(dtype=int)
        zc = g["zc"].value_counts()
        if cents:
            ds = {s: float(np.hypot(cx - c[0], cy - c[1])) for s, c in cents.items()}
            ...
            nsh_name, nsh = None, np.nan
        in_shel = float(g["roi"].isin(shel).mean()) if "roi" in g.columns else np.nan
        rows.append({"night": night, "shortid": sid, "window": window_label,
                     "n_rest_fix": int(len(g)), "centroid_x": cx, "centroid_y": cy,
                     "spread_in": spread,
                     "dominant_roi": str(rc.idxmax()) if len(rc) else "open",
                     "dominant_zone_class": str(zc.idxmax()) if len(zc) else "open",
                     "nearest_shelter": nsh_name, "dist_nearest_shelter_in": nsh,
                     "near_shelter": bool(np.isfinite(nsh) and nsh <= near_shelter_in),
                     "in_shelter_frac": in_shel})
    return pd.DataFrame(rows, columns=cols)


def nightly_activity_profile(df: pd.DataFrame, *, moving_thr_inps: float,
                             bin_s: int = 300, anchor_hour: int = 12,
                             tz_offset_hours: int = LOCAL_TZ_OFFSET_HOURS) -> pd.DataFrame:
    """
    Per **sleep-day** activity-fraction time series on a NIGHT-CENTERED axis (local
    noon of day D -> noon of D+1) — a human-readable per-night activity panel that
    captures the evening/overnight EMERGENCE without midnight truncation.

    ``sleep_day D`` groups all fixes whose local time is in
    ``[anchor_hour:00 D, anchor_hour:00 D+1)``. ``bin_hours`` = hours since midnight of
    D, so **noon = 12, midnight = 24, next-noon = 36** (past-midnight activity keeps
    counting up, never wrapping). Per ``bin_s`` bin: ``active_frac =
    mean(smoothed speed > moving_thr_inps)`` over all fixes/tags in the bin, ``n_fix``.

    ``df`` needs ``datetime`` (naive UTC) + ``speed_inps_smooth`` (post
    :func:`add_speed`); if a ``valid`` column is present only valid fixes are used.
    Long df: ``sleep_day, bin_hours, bin_local, active_frac, n_fix``.
    """
    cols = ["sleep_day", "bin_hours", "bin_local", "active_frac", "n_fix"]
    if df is None or df.empty or not {"datetime", "speed_inps_smooth"}.issubset(df.columns):
        return pd.DataFrame(columns=cols)
    d = df.copy()
    if "valid" in d.columns:
        d = d[d["valid"]]
    d = d.dropna(subset=["datetime", "speed_inps_smooth"])
    if d.empty:
        return pd.DataFrame(columns=cols)
    loc = d["datetime"] + pd.Timedelta(hours=tz_offset_hours)
    hf = loc.dt.hour + loc.dt.minute / 60.0
    # sleep_day = local date, rolled back a day for the post-midnight tail (hf < anchor)
    sday = loc.dt.normalize() - pd.to_timedelta((hf < anchor_hour).astype(int), unit="D")
    d = d.assign(_sday=sday.dt.date.astype(str))
    d["_moving"] = (d["speed_inps_smooth"] > moving_thr_inps).astype(float)
    d["bin_utc"] = _bin_utc_ns(d["datetime"], bin_s)
    rows = []
    for (sd, binu), g in d.groupby(["_sday", "bin_utc"]):
        loc_bin = pd.Timestamp(int(binu)) + pd.Timedelta(hours=tz_offset_hours)
        bh = (loc_bin - pd.Timestamp(sd)) / pd.Timedelta(hours=1)
        rows.append({"sleep_day": sd, "bin_hours": float(bh), "bin_local": loc_bin,
                     "active_frac": float(g["_moving"].mean()), "n_fix": int(len(g))})
    prof = pd.DataFrame(rows, columns=cols)
    if not prof.empty:
        prof = prof.sort_values(["sleep_day", "bin_hours"]).reset_index(drop=True)
    return prof


def sleep_emergence_from_profile(profile: pd.DataFrame, *, frac_thr: float = 0.15,
                                 sustain_bins: int = 2, search_from_h: float = 15.0
                                 ) -> pd.DataFrame:
    """
    Behavioral EMERGENCE (sleep-period end) per sleep-day from
    :func:`nightly_activity_profile` — the independent cross-check of the thermal
    :func:`temperature_calibrated_sleep_end`. **emergence_hour** = the first
    ``bin_hours >= search_from_h`` that starts a run of ``>= sustain_bins`` bins with
    ``active_frac >= frac_thr``, in hours since midnight of the sleep-day (may exceed
    24). Columns: ``sleep_day, emergence_hour, peak_active_frac``; ``emergence_hour``
    NaN if the group never reaches ``frac_thr`` (stayed at rest all night).
    """
    cols = ["sleep_day", "emergence_hour", "peak_active_frac"]
    if profile is None or profile.empty:
        return pd.DataFrame(columns=cols)
    rows = []
    for sd, g in profile.groupby("sleep_day"):
        g = g[g["bin_hours"] >= search_from_h].sort_values("bin_hours")
        af = g["active_frac"].to_numpy()
        bh = g["bin_hours"].to_numpy()
        emh = np.nan
        run = 0
        for i, a in enumerate(af):
            run = run + 1 if a >= frac_thr else 0
            if run >= sustain_bins:
                emh = float(bh[i - sustain_bins + 1])
                break
        rows.append({"sleep_day": str(sd), "emergence_hour": emh,
                     "peak_active_frac": float(np.nanmax(af)) if af.size else np.nan})
    return pd.DataFrame(rows, columns=cols)


def locomotor_emergence(profile: pd.DataFrame, *, base_lo: float = 12.0, base_hi: float = 15.5,
                        search_from: float = 15.0, rise_frac: float = 0.20, abs_margin: float = 0.03,
                        peak_hi: float = 22.0, sustain_bins: int = 3,
                        emergence_lo: float = 16.0, emergence_hi: float = 21.0) -> pd.DataFrame:
    """
    Per sleep-day **LOCOMOTOR EMERGENCE** = onset of sustained locomotion / **sleep-site
    departure**, from :func:`nightly_activity_profile`. This is the WISER-observable END of
    the daytime **SLEEP TRUNK**, and **replaces** the temperature-crossing
    :func:`temperature_calibrated_sleep_end` (which searched THROUGH the night, ran **past
    midnight** on hot days, and so conflated the active-night **midnight nap** with the
    trunk end).

    **Sensor-limited interpretation:** WISER only detects movement above the ~7 in jitter
    floor, so ``locomotor_emergence_hour`` marks the afternoon rise in *movement* off the
    rest baseline (~20:00) — i.e. when the rat starts *moving off* its sleep site. It does
    **NOT** observe in-shelter waking/stirring, so it is expected to LAG a true behavioral
    wake; the field-observed ~18:00 wake vs this ~20:00 value is **consistent with** WISER's
    invisibility to in-nest behavior but is **not proven** to be entirely caused by it
    (a genuinely later site departure is not excluded without interior CV / ephys). For a
    sleep-**SITE** window this locomotor edge is the correct boundary (that is when the site
    is vacated). It is circadian-clustered and NOT temperature-driven. Per sleep-day:

    - **baseline_active** = median ``active_frac`` over ``bin_hours ∈ [base_lo, base_hi)``
      (deep mid-afternoon rest, ~12:00–15:30 local); **dusk peak** = max over ``[search_from, peak_hi]``;
    - **threshold** = ``baseline_active + max(abs_margin, rise_frac·(peak − baseline))`` — a
      **relative** trip point ``rise_frac`` of the way up the dusk ramp (catches the emergence,
      not the near-peak an absolute margin would);
    - **locomotor_emergence_hour** = first ``bin_hours ≥ search_from`` starting a run of
      ``≥ sustain_bins`` bins ``≥ threshold``, clamped ``[emergence_lo, emergence_hi]``;
      ``crossed=False`` (→ ``emergence_hi``) if movement never rises off baseline in range.

    ``bin_hours`` on the afternoon equal the local clock hour, so ``locomotor_emergence_hour``
    is a local clock hour. Columns: ``sleep_day, baseline_active, peak_active,
    locomotor_emergence_hour, crossed``.
    """
    cols = ["sleep_day", "baseline_active", "peak_active", "locomotor_emergence_hour", "crossed"]
    if profile is None or profile.empty:
        return pd.DataFrame(columns=cols)
    rows = []
    for sd, g in profile.groupby("sleep_day"):
        g = g.sort_values("bin_hours")
        base = g[(g["bin_hours"] >= base_lo) & (g["bin_hours"] < base_hi)]["active_frac"]
        baseline = float(np.nanmedian(base)) if len(base) else np.nan
        ramp = g[(g["bin_hours"] >= search_from) & (g["bin_hours"] <= peak_hi)]["active_frac"]
        peak_ramp = float(np.nanmax(ramp)) if len(ramp) and ramp.notna().any() else np.nan
        if np.isfinite(baseline) and np.isfinite(peak_ramp):
            thr = baseline + max(abs_margin, rise_frac * (peak_ramp - baseline))
        else:
            thr = np.nan
        sub = g[g["bin_hours"] >= search_from]
        af = sub["active_frac"].to_numpy()
        bh = sub["bin_hours"].to_numpy()
        emrg = np.nan
        crossed = False
        if np.isfinite(thr) and af.size:
            run = 0
            for i, a in enumerate(af):
                run = run + 1 if a >= thr else 0
                if run >= sustain_bins:
                    emrg = float(bh[i - sustain_bins + 1])
                    crossed = True
                    break
        raw = emrg if (emrg == emrg) else emergence_hi
        rows.append({"sleep_day": str(sd), "baseline_active": baseline,
                     "peak_active": float(np.nanmax(g["active_frac"])) if len(g) else np.nan,
                     "locomotor_emergence_hour": float(min(max(raw, emergence_lo), emergence_hi)),
                     "crossed": bool(crossed)})
    return pd.DataFrame(rows, columns=cols)


# (`classify_site_location` — a coarse binary house/doorway/other classifier — was superseded
# 2026-07-11 by `classify_site_state` (full ROI state space) after a state-space-misspecification
# review; removed to avoid its reuse.)


def detect_site_changepoint(g: pd.DataFrame, *, bin_s: int = 300, smooth_bins: int = 1,
                            min_seg_bins: int = 3, min_disp_in: float = 100.0) -> dict:
    """
    **Single within-trunk change-point** in sleep-site POSITION for ONE animal-day's
    resting fixes ``g`` (needs ``datetime``, ``x``, ``y``) — estimated **independently**, no
    fixed switch hour imposed. Per ``bin_s`` bin: median (x, y) (robust to jitter); optional
    centered rolling-median smoothing over ``smooth_bins``. For every split index with
    ``≥ min_seg_bins`` bins on each side, the pre/post **segment-median** centroids give a
    displacement; the split maximizing displacement is the change-point.

    A change-point is ``supported`` if ``displacement ≥ min_disp_in`` AND both segments have
    ``≥ min_seg_bins`` bins. ``confidence = displacement / (displacement + pooled within-segment
    scatter)`` ∈ [0,1] (higher = a cleaner step vs noise). Returns a dict with ``n_bins,
    present_bins, cp_time_utc (ns, first bin of the post-segment), displacement_in,
    within_scatter_in, confidence, supported, pre_x/pre_y, post_x/post_y``. ``supported=False``
    (NaN cp) if too few bins.
    """
    out = {"n_bins": 0, "present_bins": 0, "cp_time_utc": np.nan, "displacement_in": np.nan,
           "within_scatter_in": np.nan, "confidence": np.nan, "supported": False,
           "pre_x": np.nan, "pre_y": np.nan, "post_x": np.nan, "post_y": np.nan}
    d = g.dropna(subset=["x", "y", "datetime"]).copy()
    if len(d) < 2 * min_seg_bins:
        return out
    d["bin_utc"] = _bin_utc_ns(d["datetime"], bin_s)
    b = d.groupby("bin_utc").agg(x=("x", "median"), y=("y", "median")).sort_index()
    out["present_bins"] = int(len(b))
    if smooth_bins > 1:
        b["x"] = b["x"].rolling(smooth_bins, center=True, min_periods=1).median()
        b["y"] = b["y"].rolling(smooth_bins, center=True, min_periods=1).median()
    n = len(b)
    out["n_bins"] = int(n)
    if n < 2 * min_seg_bins:
        return out
    xs, ys = b["x"].to_numpy(), b["y"].to_numpy()
    times = b.index.to_numpy()
    best_i, best_d = -1, -1.0
    for i in range(min_seg_bins, n - min_seg_bins + 1):
        px, py = np.median(xs[:i]), np.median(ys[:i])
        qx, qy = np.median(xs[i:]), np.median(ys[i:])
        disp = float(np.hypot(qx - px, qy - py))
        if disp > best_d:
            best_d, best_i = disp, i
    px, py = float(np.median(xs[:best_i])), float(np.median(ys[:best_i]))
    qx, qy = float(np.median(xs[best_i:])), float(np.median(ys[best_i:]))
    scat = float(np.median(np.concatenate([
        np.hypot(xs[:best_i] - px, ys[:best_i] - py),
        np.hypot(xs[best_i:] - qx, ys[best_i:] - qy)])))
    out.update({"cp_time_utc": int(times[best_i]), "displacement_in": best_d,
                "within_scatter_in": scat,
                "confidence": float(best_d / (best_d + scat)) if (best_d + scat) > 0 else np.nan,
                "supported": bool(best_d >= min_disp_in),
                "pre_x": px, "pre_y": py, "post_x": qx, "post_y": qy})
    return out


# --- Multi-site sleep-SITE state space (full ROI set, not binary house_1/house_2) ----------
# assign_roi labels -> canonical shelter-type STATES; food_1/2 sit INSIDE house_1/2 so map to
# the house; 'edge'/'open' are handled as doorway/exposed by the distance rule below.
CANON_SITE_STATE = {
    "house_1": "house_1", "food_1": "house_1",
    "house_2": "house_2", "food_2": "house_2",
    "refuge_1": "refuge_1", "refuge_2": "refuge_2", "refuge_3": "refuge_3", "refuge_4": "refuge_4",
    "water_1": "water_1", "water_2": "water_2", "tunnel_1": "tunnel",
}
SHELTER_STATES = ("house_1", "house_2", "refuge_1", "refuge_2", "refuge_3", "refuge_4",
                  "water_1", "water_2", "tunnel")


def _roi_active_on(roi: dict, date) -> bool:
    """Is a time-varying ROI present on local ``date`` (coarse day-level `valid_from/until`)?"""
    vf, vu = roi.get("valid_from"), roi.get("valid_until")
    if date is None or not (vf or vu):
        return True
    d = str(date)[:10]
    if vf and d < str(vf)[:10]:
        return False
    if vu and d > str(vu)[:10]:
        return False
    return True


def _roi_edge_distance(cx: float, cy: float, roi: dict) -> float:
    """Distance (in) from a point to an ROI footprint EDGE (0 if inside). Circle or
    (rotated) rect."""
    if roi.get("shape") == "rect":
        th = np.radians(roi.get("orientation_deg", 0.0))
        dx, dy = cx - roi["x"], cy - roi["y"]
        c, s = np.cos(-th), np.sin(-th)
        lx, ly = abs(c * dx - s * dy), abs(s * dx + c * dy)
        return float(np.hypot(max(0.0, lx - roi.get("width_in", 10.0) / 2),
                              max(0.0, ly - roi.get("height_in", 10.0) / 2)))
    return float(max(0.0, np.hypot(cx - roi["x"], cy - roi["y"]) - roi.get("radius_in", 10.0)))


def classify_site_state(cx: float, cy: float, roi_cfg: dict | None, *, date=None,
                        shelter_buffer_in: float = 15.0, doorway_buffer_in: float = 24.0) -> str:
    """
    Canonical **sleep-site STATE** of a segment centroid over the FULL ROI state space —
    house_1, house_2, refuge_1/2/3, refuge_4 (date-gated by `valid_until`), water_1, water_2,
    tunnel, **doorway** (near a shelter core but outside it), **exposed** (open / far), or
    **unknown** (NaN centroid). **Explicit distance rule** (absorbs the ~7 in jitter, p95 ~15):
    a shelter is 'entered' when the centroid is within its footprint + ``shelter_buffer_in``;
    a doorway band extends a further ``doorway_buffer_in``; else exposed. `food_1/food_2` map to
    the house they sit inside. (No dedicated water-tower ROI; the boundary rect is inconsistent
    with the house positions, so no reliable 'perimeter' state — doorway is the entrance proxy.)
    """
    if roi_cfg is None or not (np.isfinite(cx) and np.isfinite(cy)):
        return "unknown"
    best_state, best_d = None, np.inf
    for roi in roi_cfg.get("rois", []):
        st = CANON_SITE_STATE.get(roi["name"])
        if st is None or not _roi_active_on(roi, date):
            continue
        d = _roi_edge_distance(float(cx), float(cy), roi)
        if d < best_d:
            best_d, best_state = d, st
    if best_state is not None:
        if best_d <= shelter_buffer_in:
            return best_state
        if best_d <= shelter_buffer_in + doorway_buffer_in:
            return "doorway"
    return "exposed"


def trunk_state_dwell_transitions(g: pd.DataFrame, roi_cfg: dict | None, *, date=None,
                                  bin_s: int = 300, min_dwell_bins: int = 3, min_disp_in: float = 24.0,
                                  shelter_buffer_in: float = 15.0, doorway_buffer_in: float = 24.0):
    """
    Multi-site **state sequence** of ONE animal-day's trunk resting fixes ``g`` (needs
    ``datetime``, ``x``, ``y``). Bin to ``bin_s`` median positions; per bin ->
    :func:`classify_site_state`; collapse consecutive same-state runs into segments. Returns
    ``(dwell, relocations, segments)``:

    - ``dwell`` = ``{state: n_bins}`` over all bins (dwell time = ``n_bins·bin_s``);
    - ``relocations`` = list of ``{time_utc, from_state, to_state, disp_in}`` for transitions
      between two segments that **both** have ``n_bins >= min_dwell_bins``, differ in state, and
      whose centroids differ by ``>= min_disp_in`` — labels come from **independent** state
      classification, never from displacement direction; brief flickers are skipped;
    - ``segments`` = the collapsed ``{state, start_utc, end_utc, n_bins, cx, cy}`` list.
    """
    empty: tuple = ({}, [], [])
    d = g.dropna(subset=["x", "y", "datetime"]).copy()
    if len(d) < min_dwell_bins:
        return empty
    d["bin_utc"] = _bin_utc_ns(d["datetime"], bin_s)
    b = d.groupby("bin_utc").agg(x=("x", "median"), y=("y", "median")).sort_index()
    b["state"] = [classify_site_state(r.x, r.y, roi_cfg, date=date,
                                      shelter_buffer_in=shelter_buffer_in,
                                      doorway_buffer_in=doorway_buffer_in) for r in b.itertuples()]
    dwell = {str(k): int(v) for k, v in b["state"].value_counts().to_dict().items()}
    times, states = b.index.to_numpy(), b["state"].to_numpy()
    xs, ys = b["x"].to_numpy(), b["y"].to_numpy()
    segs, i, n = [], 0, len(b)
    while i < n:
        j = i
        while j + 1 < n and states[j + 1] == states[i]:
            j += 1
        segs.append({"state": str(states[i]), "start_utc": int(times[i]), "end_utc": int(times[j]),
                     "n_bins": int(j - i + 1), "cx": float(np.median(xs[i:j + 1])),
                     "cy": float(np.median(ys[i:j + 1]))})
        i = j + 1
    conf = [s for s in segs if s["n_bins"] >= min_dwell_bins]
    relocations = []
    for a, c in zip(conf[:-1], conf[1:]):
        if a["state"] != c["state"]:
            disp = float(np.hypot(c["cx"] - a["cx"], c["cy"] - a["cy"]))
            if disp >= min_disp_in:
                relocations.append({"time_utc": int(c["start_utc"]), "from_state": a["state"],
                                    "to_state": c["state"], "disp_in": disp})
    return dwell, relocations, segs


# --- Sleep-site landmark HIERARCHY primitives (state ROLE in the decision process) ----------
# Pure, testable helpers for analyze_sleep_site_hierarchy.py: do the daytime rest states have
# DIFFERENT roles (anchor / sink / home-base) or are they EXCHANGEABLE? Descriptive semi-Markov
# level ONLY (no reward/IRL — identifiability). Aggregation + permutation loops live in the driver;
# these are the primitives the self-test plants and checks.

def net_flux_scores(trans_df: pd.DataFrame) -> pd.DataFrame:
    """
    Per-state **net-flux (sink) score** from a relocation transition table ``trans_df``
    (columns ``from_state, to_state, n``). For state s: ``In(s)`` = Σ arrivals (n into s),
    ``Out(s)`` = Σ departures (n out of s), and ``net_flux = (In − Out)/(In + Out)`` ∈ [−1, 1]
    — **>0 = net sink** (entered more than left → an attractor/return state), **<0 = net source**.
    Returns ``state, arrivals, departures, net_flux`` sorted by net_flux desc. Empty → empty.
    """
    cols = ["state", "arrivals", "departures", "net_flux"]
    if trans_df is None or trans_df.empty:
        return pd.DataFrame(columns=cols)
    inc = trans_df.groupby("to_state")["n"].sum()
    out = trans_df.groupby("from_state")["n"].sum()
    rows = []
    for s in sorted(set(inc.index) | set(out.index)):
        a, d = float(inc.get(s, 0.0)), float(out.get(s, 0.0))
        nf = (a - d) / (a + d) if (a + d) > 0 else np.nan
        rows.append({"state": s, "arrivals": int(a), "departures": int(d), "net_flux": nf})
    return pd.DataFrame(rows, columns=cols).sort_values("net_flux", ascending=False, ignore_index=True)


def anchor_concentration_kl(anchor_counts: dict, dwell_weights: dict) -> float:
    """
    **Non-exchangeability statistic** for the anchor (or terminal) role:
    ``D = Σ_s α(s)·log(α(s)/w(s))`` — the KL divergence of the observed start-state distribution
    ``α`` (``anchor_counts`` normalized) from the **dwell-weighted expectation** ``w``
    (``dwell_weights`` normalized). ``D ≈ 0`` → day-starts occur in proportion to occupancy (states
    exchangeable **given occupancy**); ``D ≫ 0`` → some states anchor **more than occupancy
    predicts** (a special role). ``α(s)=0`` contributes 0; ``w(s)=0`` with ``α(s)>0`` is floored at
    ``1e-9``. Returns ``D`` (nats); NaN if either total is non-positive.
    """
    tot_a = float(sum(anchor_counts.values())); tot_w = float(sum(dwell_weights.values()))
    if tot_a <= 0 or tot_w <= 0:
        return np.nan
    D = 0.0
    for s, c in anchor_counts.items():
        a = c / tot_a
        if a <= 0:
            continue
        wv = dwell_weights.get(s, 0.0) / tot_w
        D += a * np.log(a / (wv if wv > 0 else 1e-9))
    return float(D)


def permutation_pvalue(observed: float, null_samples, tail: str = "right") -> float:
    """
    Monte-Carlo permutation **p-value** with +1 smoothing:
    ``p = (1 + #{null tail-exceeds observed}) / (1 + n_null)``. ``tail='right'`` counts
    ``null ≥ observed`` (concentration / agreement statistics), ``'left'`` counts ``null ≤ observed``.
    NaN if no finite null samples or non-finite ``observed``.
    """
    ns = np.asarray([x for x in null_samples if x == x], float)
    if ns.size == 0 or not np.isfinite(observed):
        return np.nan
    k = int(np.sum(ns <= observed)) if tail == "left" else int(np.sum(ns >= observed))
    return float((1 + k) / (1 + ns.size))


def kendall_w(rank_matrix) -> float:
    """
    **Kendall's coefficient of concordance** ``W`` ∈ [0,1] across ``m`` judges (rows) ranking ``k``
    items (columns): ``W = 12·S / (m²·(k³ − k))`` with column rank sums ``R_j`` and
    ``S = Σ_j (R_j − mean R)²``. ``W=1`` identical rankings, ``W=0`` none. Average-rank ties allowed;
    uncorrected form (permutation null carries the inference). Needs ``m ≥ 2, k ≥ 2`` else NaN.
    """
    M = np.asarray(rank_matrix, float)
...
# --- Threshold / gate + clustered-bootstrap primitives (heat-gated relocation) --------------
# Pure, testable helpers for analyze_heat_gated_relocation.py: fit a 1-D logistic dose-response
# (temperature -> P(state)), read off its threshold, and put a DAY-CLUSTERED bootstrap CI on an
# effect (rats within a day share the weather, so days are the independent units).

def logistic_fit_1d(x, y, *, iters: int = 50, l2: float = 1e-4):
    """
    Fit a 1-D **logistic** ``P(y=1|x) = σ(b0 + b1·x)`` by ridge-regularized IRLS (Newton).
    ``x`` = predictor (e.g. temperature), ``y`` = 0/1 outcome. ``l2`` ridge on the slope keeps it
    finite under separation. Returns ``(b0, b1)`` (NaNs if <4 points, no variance, or one-class ``y``).
    """
    x = np.asarray(x, float); y = np.asarray(y, float)
    m = np.isfinite(x) & np.isfinite(y); x, y = x[m], y[m]
    if len(x) < 4 or np.ptp(x) == 0 or len(np.unique(y)) < 2:
        return (np.nan, np.nan)
    xs = (x - x.mean()) / (x.std() or 1.0)                    # standardize for conditioning
    b = np.zeros(2); X = np.column_stack([np.ones_like(xs), xs])
    R = np.diag([0.0, l2])
    for _ in range(iters):
        ...
        if np.max(np.abs(step)) < 1e-8:
            break
    b1 = b[1] / (x.std() or 1.0)                              # de-standardize
    b0 = b[0] - b1 * x.mean()
    return (float(b0), float(b1))


def logistic_threshold(b0: float, b1: float, level: float = 0.5) -> float:
    """Temperature (x) at which the logistic ``σ(b0+b1·x)`` crosses ``level``:
    ``x = (logit(level) − b0) / b1``. NaN if ``b1≈0`` or inputs non-finite."""
    if not (np.isfinite(b0) and np.isfinite(b1)) or abs(b1) < 1e-12 or not (0 < level < 1):
        return np.nan
    return float((np.log(level / (1 - level)) - b0) / b1)


def cluster_bootstrap(groups, *, n_boot: int = 2000, seed: int = 0, agg=np.mean) -> dict:
    """
    **Day-clustered bootstrap** of a pooled statistic. ``groups`` = list of arrays, one per cluster
    (e.g. one day's per-rat-day Δ values). Each replicate resamples the CLUSTERS with replacement,
    pools their values, and applies ``agg`` (default mean). Returns ``{observed, mean, lo, hi
    (2.5/97.5 pct), frac_gt0, n_clusters}``. Clustering by day respects that rats within a day share
    weather (days, not rat-days, are independent). NaN dict if no data.
    """
    gs = [np.asarray(g, float)[np.isfinite(np.asarray(g, float))] for g in groups]
    gs = [g for g in gs if g.size]
    out = {"observed": np.nan, "mean": np.nan, "lo": np.nan, "hi": np.nan,
           "frac_gt0": np.nan, "n_clusters": len(gs)}
    if not gs:
        return out
    pooled = np.concatenate(gs)
    out["observed"] = float(agg(pooled))
    rng = np.random.default_rng(seed)
    idx = np.arange(len(gs))
    stats = np.empty(n_boot)
    for i in range(n_boot):
        pick = rng.choice(idx, size=len(gs), replace=True)
        stats[i] = agg(np.concatenate([gs[j] for j in pick]))
    out.update(mean=float(np.mean(stats)), lo=float(np.percentile(stats, 2.5)),
               hi=float(np.percentile(stats, 97.5)), frac_gt0=float(np.mean(stats > 0)))
    return out
