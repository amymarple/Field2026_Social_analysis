...
D:\Field2026_analysis_out\direction3_temperature_relocation_<ts>\; the report is also written to
...
# Newest snapshot on the analysis PC → full daytime window 2026-06-28 → 07-08.
...
DEFAULT_WEATHER_DIR = Path(r"D:\Reolink_record\audio_in\weather_data")
# Auto-discover ALL AWN exports; load_weather_multi dedups on datetime_utc, so overlapping
# cumulative exports are safe and any newly-dropped file (e.g. a wider date range) is picked
# up automatically — no need to edit this when the weather export is refreshed.
DEFAULT_WEATHER = (sorted(DEFAULT_WEATHER_DIR.glob("AWN-*.csv"))
                   if DEFAULT_WEATHER_DIR.exists() else [])
...
# refuge_4 is a BURROW ENTRANCE, not a sleep site, in this window (dig ~07-03 01:00 EDT
# nightly by >1 rat; removed 07-07 13:00). Rest reads there are burrow-entrance behaviour
# + a UWB-dropout lower bound (tag below the anchor plane), NEVER daytime sleep. See
# change_log/2026-07-07-shelter4-burrow-removed.md.
BURROW_ROI = "refuge_4"
# dig ~07-03 01:00 → removal 07-07 13:00; end-exclusive bound is 07-08 so 07-07 morning
# (pre-removal) refuge_4 reads are still flagged as burrow (valid_until drops it after 13:00).
BURROW_WINDOW = ("2026-07-03", "2026-07-08")
...
def _burrow_reads(seq: pd.DataFrame) -> pd.DataFrame:
    """(night, shortid, window) rows where the dominant rest ROI is the refuge_4 BURROW
    entrance inside the burrow window — NOT sleep; a UWB-dropout lower bound."""
    if seq.empty or "dominant_roi" not in seq.columns:
        return seq.iloc[:0]
    lo, hi = BURROW_WINDOW
    m = (seq["dominant_roi"] == BURROW_ROI) & (seq["night"] >= lo) & (seq["night"] < hi)
    return seq[m][["night", "shortid", "window", "dominant_roi"]].reset_index(drop=True)
...
def _build_report(bouts, seq, events, conv, dropout, reloc_summary, nights, tags,
    ...
    L.append(f"- Animal-days with >25% dropout: {len(hi)}"
             ...
             + ". (6/28 is a PARTIAL day — evening release ~19:25 — so its high dropout is expected, "
             "not signal loss.) Interpret high-dropout reads as **lower-confidence**.\n")

    # refuge_4 burrow caveat (07-03 -> 07-07): refuge_4 is a burrow ENTRANCE, not sleep
    br = _burrow_reads(seq)
    L.append(f"## refuge_4 BURROW caveat ({BURROW_WINDOW[0]} → 2026-07-07 13:00 removal, NOT sleep)\n")
    if br.empty:
        L.append("- No dominant-`refuge_4` rest windows in the burrow window (nothing to reinterpret).\n")
    else:
        who = ", ".join(f"{r.night[5:]}/{r.shortid}({r.window.split('_')[0]})" for r in br.itertuples())
        L.append(f"- **{len(br)} animal-window(s)** show `refuge_4` as the dominant daytime rest ROI "
                 f"inside the burrow window: {who}. `refuge_4` was a **burrow ENTRANCE** (>1 rat dug it "
                 "nightly from ~07-03 01:00 EDT; removed 07-07 13:00), so these are **burrow-entrance "
                 "behaviour + a UWB-dropout lower bound** (the tag vanishes when the rat goes below the "
                 "anchor plane), **never daytime sleep**. Do NOT count them as a shelter/rest site or a "
                 "'relocation to refuge_4'. All house_1/house_2 reads are unaffected. (See "
                 "`change_log/2026-07-07-shelter4-burrow-removed.md`.)\n")
    ...
    L.append(f"**Observed within-day pattern (computed, descriptive over {len(nights)} days):**")
    for night in nights:
    ...
    # data-driven multi-day summary: how often does the late-morning shelter aggregation
    # thin out at the midday heat peak (fewer animals in a shelter than late-morning)?
    thinned, comparable = [], 0
    for night in nights:
        c = conv[conv["night"] == night].set_index("window")
        lm = c["n_in_shelter"].get("late_morning", np.nan)
        mh = c["n_in_shelter"].get("midday_heat", np.nan)
        if lm == lm and mh == mh:
            comparable += 1
            if mh < lm:
                thinned.append(night)
    L.append(f"\n**Cross-day summary (the actual multi-day pattern):** on {len(thinned)}/{comparable} "
             f"days with both windows present, FEWER animals are in a shelter at the 12:00-15:00 heat "
             f"peak than in late morning ({', '.join(d[5:] for d in thinned) if thinned else 'none'}) — "
             "i.e. the late-morning shelter aggregation tends to **thin out / disperse at the heat "
             "peak** (rats move to house_2 and/or the open field), rather than pile deeper into one "
             "shelter. This is the repeated, candidate **temperature-linked** signature; it is "
             "DESCRIPTIVE (outside-air-temp proxy, no shelter thermistor) and does NOT establish "
             "causation, nor is house_2 verified cooler (inch frame).\n")
    ...
    L.append("- Candidate interpretations (not mutually exclusive): **wet-day convergence** (rain "
             ...
             "baseline/most-common site for most animals anyway). WISER cannot separate sleep state or true "
    ...
    # answers (data-driven over all days present)
    sw = events[events.get("kind", "") == "shelter_switch"] if not events.empty else events
    sw_animals = sorted({str(r.shortid) for r in sw.itertuples()}) if not sw.empty else []
    n_sw = int(len(sw))
    L.append("## Direct answers\n")
    L.append("1. **Across-day relocation:** see Stage A `rest_site_stability.csv` tiers "
             "(`analyze_daytime_sleep_site.py`) — over the full 8-day window ALL 5 animals switch "
             "house_1↔house_2 at least once, but at **heterogeneous rates** (12407 nearly every day; "
             "12378/12380/12386 mostly house_1-faithful with late/early switches). The 3-day 'only "
             "12386 & 12407' read was a small-sample artifact.")
    L.append(f"2. **Within-day house_1↔house_2 switching is common, not rare:** {n_sw} within-day "
             f"shelter-switch events over {len(nights)} days, involving "
             f"{', '.join(sw_animals) if sw_animals else 'no'} animals — i.e. broader than the 2 "
             "animals seen across-day. Within-day site change is the norm; across-day *identity* "
             "change is the rarer signal.")
    L.append("3. **Within-day moves:** see the per-day sequences above (ROI change across windows), "
             "excluding refuge_4 burrow reads (flagged above).")
    L.append(f"4. **Temperature regularity:** reported as entropy/share vs window+temp — descriptive "
             f"over {len(nights)} days; labelled temperature-**linked** at most (outside-air proxy).")
    L.append("5. **Heat-peak dispersal:** the repeated signature is the late-morning shelter "
             "aggregation THINNING at the midday heat peak (see the cross-day summary above), "
             "quantified with the dropout guard so it is not a missing-data artifact.")
    L.append("6. **Best interpretation:** candidate / measurement-limited — WISER supports site-level "
             "within-day movement and repeated midday shelter-dispersal; thermal vs social vs habit "
             "cannot be separated without a shelter thermistor / ephys, and the inch frame is "
             "unverified (house_2 not confirmed cooler).\n")
    L.append(f"\n*Figures + CSVs: `{out}`.*\n")
    return "\n".join(L)
