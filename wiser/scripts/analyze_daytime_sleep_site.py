...
Read-only on D:\Wiser\data; outputs to D:\Field2026_analysis_out\daytime_sleep_site_YYYYMMDD_HHMM\.
...
def main() -> None:
    ...
    # --- verdict: per-animal switch RATE (robust when many/all animals switch) ---
    ...
    # per-animal switch rate = (# house_1<->house_2 switches) / (# consecutive-day transitions)
    if not stab.empty and "shelter_switch" in stab:
        srate = (stab.assign(sw=stab["shelter_switch"].astype(bool))
                 .groupby(stab["shortid"].astype(str))
                 .agg(n_switch=("sw", "sum"), n_trans=("sw", "size")))
        srate = srate.sort_values(["n_switch", "n_trans"], ascending=False)
        rate_str = "; ".join(f"{sid} {int(r.n_switch)}/{int(r.n_trans)}"
                             for sid, r in srate.iterrows())
        n_any = int((srate["n_switch"] > 0).sum())
        n_tot = int(srate.shape[0])
        het = (f"Across-day shelter fidelity is HETEROGENEOUS BY RATE (house_1<->house_2 switches / "
               f"consecutive-day transitions): {rate_str}. {n_any}/{n_tot} animals switch at least "
               f"once; the switch RATE — not a stable/relocator dichotomy — is the real signal "
               f"(the earlier 3-day 'only 2 animals relocate' read was a small-sample artifact). "
               f"Switches are genuine ~185-212 in house-to-house moves (~ the 5 ft house spacing), "
               f"far above the ~{jitter:.0f} in jitter floor.")
    else:
        het = "No across-day transitions available."
    verdict = (
        ...
        f"(jitter floor ~{jitter:.0f} in): {tier_counts}. {het} Note the primary daytime site is "
        f"house_1/house_2/open only — refuge_4 never dominates a full day, so the burrow window "
        f"(07-03->07-07) does NOT contaminate these across-day switches. Sleep = low-speed proxy "
        f"(< {moving_thr:.1f} in/s), NOT ephys-validated; WISER frame "
        f"{'georeferenced' if transform else 'UNVERIFIED'} (inch offset); ROI names "
        f"{'from confirmed ROIs' if roi_cfg else 'unavailable'} (provisional). CV shelter cams "
        f"corroborate visible shelter-resident periods only (CV = lower bound; 2026-07-06 "
        f"reconciliation). Only site differences >> the ~{jitter:.0f} in jitter floor are trustworthy.")
