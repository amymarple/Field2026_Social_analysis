...
def main() -> None:
    ap = argparse.ArgumentParser(description="Nightly 9pm-12am behavior & social (nights auto-discovered).")
    ...
    verdict = (
        ...
        f"Compares night 1 ({nights[0]}) -> night {len(nights)} ({nights[-1]}). Known rain (6/30 ~17:20, "
        f"7/1 evening, 7/7 overnight) is a covariate on the settling trend -> weather vs habituation not "
        f"separated here; see analyze_nightly_progression.py for per-night rain classification. Tunnel "
        f"present 6/28 only.")
