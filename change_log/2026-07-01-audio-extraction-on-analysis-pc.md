...
- `audio/src/audio_io.py` — **new** `_dedup_overlapping()`, called by
...
- `audio/scripts/selftest_features.py` — added offline dedup checks (the 6/29 14:00
  nested-backfill scenario drops exactly the two covered segments; a contiguous chain is unchanged).
- `audio/src/plotting.py` — `plot_level_over_time` now annotates the level plot with a
...
- `audio/src/plotting.py` + `scripts/summarize_soundscape.py` — new
...
- `audio/environment.yml` — added `- ffmpeg` (conda-forge) so the `audio` env is
  self-sufficient here (the field PC still uses its own `E:\Reolink_record\bin\ffmpeg.exe`;
  additive, no behavior change there).
- `audio/configs/audio_analysis.analysis_pc.yaml` — **new** machine-specific config; a
...
- Before→after dedup, the `ok`-based Leq/index medians are identical to 3 dp — the science was
  ...
  `timeline_gap`).
- Outputs (git-ignored `audio/outputs/`): `audio_features_CH0{1,2}_2026-06-29.csv`
...
- **The 6/29 ~15:00–17:45 audio gap is REAL and not recoverable from the NVR backfill.** During the
  ...
  the NVR IP-change recording gap (documented in the `social_recording` repo):
