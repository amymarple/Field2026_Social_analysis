...
`cv/measurement_context.py` makes every shelter number interpretable as a
...
`cv/fog_risk.py` — a MEASUREMENT-CONTEXT covariate (not behavior): from AWN
...
logs. Consolidated them into a single human reference, **`cv/LABELING_PROTOCOL.md`**,
...
(`audio/analysis/weather.py:load_awn`).
...
`cv/glass_regime.py`: `load_regimes()`, `regime_at(ts, channel=None)` (returns the
