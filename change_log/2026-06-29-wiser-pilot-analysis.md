...
- Added `wiser/src/wiser_analysis_utils.py` — reusable pilot layer on top of
...
- Added `wiser/scripts/place_wiser_rois.py` — matplotlib ROI **editor**,
...
- **ROI shapes:** circle (refuge/water/food) and **rect** (the 2 big rectangular houses). Rect house
  ...
  `cv/configs/field_layout.json` (`shelters`); only the size is reused —
...
- Added `wiser/notebooks/wiser_pilot_analysis.ipynb` — QC-first Jupyter notebook
...
- Added `wiser/configs/wiser_rois.json` — provisional placeholder (confirmed=false).
...
Outputs written to `D:\Field2026_analysis_out\wiser_pilot_output_YYYYMMDD_HHMM\` (off C:, off git, never
