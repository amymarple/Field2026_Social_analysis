...
**Status tracker:** [wiser/ANALYSIS_STATUS.md](../wiser/ANALYSIS_STATUS.md) (P0 blocker).
...
- **New `wiser/src/field_transform.py`** — pure-numpy transform core:
  ...
  (WISER inch → field cm).
- **New `wiser/scripts/georeference_wiser.py`** — CLI: reads the pole survey,
...
- **New `wiser/scripts/selftest_georeference.py`** — offline verification (no
  DB/field data).
- **New `wiser/configs/wiser_georef_survey.csv`** — survey input template
  (commented examples; parses to zero rows until filled).
- **Edited `wiser/src/wiser_analysis_utils.py`** — added `load_field_transform`
...
- **Edited `wiser/.gitignore`** — ignore `outputs/*.png` (the QC overlay is
  derived data).
