...
**(a) Rain-/broadband-aware biophony heuristic** — `audio/src/plotting.py`:
...
- New `audio/analysis/wiser_activity.py` — `hourly_rat_activity(dates)` reuses the
  `wiser` pipeline **read-only** (`load_wiser_session` → `convert_timestamps` →
