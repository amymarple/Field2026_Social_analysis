...
New top-level `audio/` subsystem (mirrors `wiser/`) that extracts
...
- `.gitignore` — ignore `audio/outputs/` (derived data).
...
Raw MP4s untouched (read-only); all output under git-ignored `audio/outputs/`.
