...
- Added `wiser/scripts/plot_hourly_occupancy.py`:
...
- Added `wiser/install_wiser_occupancy_task.ps1` — registers an hourly SYSTEM
  scheduled task running the script in `--backfill` mode.
- Added `wiser/README_occupancy.md`.
...
PNGs default to `D:\Field2026_analysis_out` (off the C: drive, outside the git repo; the `arena_extent.json`
...
- Added `wiser/configs/rat_identities.csv` (the explicit mapping CLAUDE.md calls
