...
import stripe_flow as sf          # DIAGNOSTIC-ONLY optical-flow motion index Phi; feeds no decision
...
def score_stripe_flow(samples, cfg):
    """Annotate each sample with the stripe-flow motion index Phi (cov, coh), computed on the SAME
    human-review CLIP that then gets labeled. DIAGNOSTIC ONLY: never sets/overrides pred_state or
    any safety decision -- it only adds two covariate columns so the report can cross-tab Phi
    against the human 'moving under fog?' label (the ground-truth positive class for feasibility).
    A MOTION LOWER BOUND; low Phi never implies absence/stillness."""
    zones_cache, mask_cache = {}, {}
    v_min = float(cfg.get("stripe_flow_vmin", 1.0))
    dark_pctl = float(cfg.get("stripe_flow_dark_pctl", 25.0))
    for s in samples:
        grays = [g for g in (ss.read_gray(p) for p in (s.get("clip") or s["burst"])) if g is not None]
        if len(grays) < 2:
            s["stripe_flow_cov"] = s["stripe_flow_coh"] = float("nan")
            continue
        mask = _inside_mask(s["channel"], grays[0].shape, zones_cache, mask_cache)
        cov, coh = sf.stripe_flow(grays, mask, v_min=v_min, dark_pctl=dark_pctl)
        s["stripe_flow_cov"] = round(cov, 4)
        s["stripe_flow_coh"] = round(coh, 3)
...
def report(samples, date, cfg, out_csv, run_id=None, context=None):
    ...
    df = pd.DataFrame([{k: s.get(k) for k in ("channel", "file", "t", "view_quality_inside", "pred_count",
                                              ...
                                              "stripe_flow_cov", "stripe_flow_coh")}
                       for s in rows])
    ...
    # ---- Phi (stripe-flow motion index) vs YOUR labels: DIAGNOSTIC ONLY, feeds no decision ----
    # Feasibility question: does coherent dark-pixel flow fire on a *moving rat under fog* (target
    # positive) and stay low on fog-only/empty and still rats? This is a MOTION lower bound, not a
    # detector; a low Phi never implies absence/stillness (occlusion / blind zone / missed instant).
    if "stripe_flow_cov" in df and df["stripe_flow_cov"].notna().any():
        phi = df[df["stripe_flow_cov"].notna()].copy()
        clear = phi["gt_view"] == "clear"
        occ = phi["gt_count"] > 0
        moving = phi["gt_motion"] == "moving"
        classes = [
            ("CLEAR-ctrl | moving (positive control)", clear & occ & moving),
            ("CLEAR-ctrl | still",                     clear & occ & ~moving),
            ("CLEAR-ctrl | empty",                     clear & ~occ),
            ("FOG/DEGRADED | moving (TARGET)",         ~clear & occ & moving),
            ("FOG/DEGRADED | still",                   ~clear & occ & ~moving),
            ("FOG/DEGRADED | empty",                   ~clear & ~occ),
        ]
        print(f"\nPhi stripe-flow (DIAGNOSTIC, motion lower bound; n={len(phi)} scored clips):")
        print("    class                                    n   cov mean (max)     coh mean (max)")
        for name, m in classes:
            g = phi[m]
            if not len(g):
                continue
            print(f"    {name:<40} {len(g):>2}   "
                  f"{g['stripe_flow_cov'].mean():.4f} ({g['stripe_flow_cov'].max():.4f})   "
                  f"{g['stripe_flow_coh'].mean():.3f} ({g['stripe_flow_coh'].max():.3f})")
        print("    feasible IF 'moving' classes (esp. CLEAR-ctrl control, and the FOG TARGET) read "
              "HIGHER cov/coh than 'still'/'empty'.")
...
def main() -> None:
    ...
    score_stripe_flow(samples, cfg)   # DIAGNOSTIC-only Phi on the review clip; adds covariate columns
