...
**Generated (UTC):** 2026-07-10T04:26:23.687875  
...
- **2046 strict-following episodes** across 11 nights (median 3 s, p95 9 s). Per-night counts in `incident_summary_by_night.csv`; per (night, ordered pair) rates in `incident_metrics_by_pair_night.csv`.
...
|---|---|---|---|
| Sen → Siesta | 1.82 | 0.02 | 157 |
| Sen → Hypnos | 1.70 | 0.02 | 146 |
| Hypnos → Nox | 1.47 | 0.02 | 127 |
| Sen → Dormi | 1.47 | 0.02 | 122 |
| Siesta → Hypnos | 1.42 | 0.02 | 122 |
| Sen → Nox | 1.41 | 0.02 | 121 |
| Hypnos → Siesta | 1.33 | 0.01 | 114 |
| Siesta → Nox | 1.32 | 0.02 | 114 |
| Nox → Siesta | 1.30 | 0.01 | 112 |
| Nox → Hypnos | 1.25 | 0.01 | 107 |
...
- **2046/2046** episodes routed to a channel; **1545** flagged near a channel boundary (pull ≥2 channels). Queue: `strict_following_video_queue.csv` (sorted by routing confidence then duration).
