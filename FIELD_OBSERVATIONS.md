...
Source of truth for identity + validity: `wiser/configs/rat_identities.csv`.
...
- **Analysis hooks (done):** `valid_until = 2026-07-09T03:35:41-04:00` added for `12380` in
  `configs/rat_identities.csv`, so `apply_tag_cutoffs` flags/drops Hypnos fixes at/after the drop and
  excludes it from the `valid` set. **Existing analyses are UNAFFECTED** — all current WISER results
  (Direction 3 through 07-08; the trajectory/following/motif work through 07-05) end **before** the
  drop, so Hypnos data in those windows is real. Only analyses that extend into **07-09 03:35+** must
  apply the cutoff (any pooling across it must not treat post-drop Hypnos as an animal).
- **Interpretation caution:** near the drop, expect Hypnos fixes to go stationary / jittery / gap-out
  as the implant detaches — do **not** read a "sudden stop" or "left the group" around 03:35 as
  behavior; it is the sensor-path (implant failure), classified measurement-artifact.

- **Small chair left in the paddock ~12:15 EDT 2026-07-09** (forgotten during the Hypnos removal). A
  new **static object enters the paddock / camera field of view from ~12:15 07-09 onward** — treat as
  a **covariate / regime boundary, not an exclusion**: (a) CV — a fixed background object that may draw
  false detections, occlude, or read as occupancy; (b) WISER — a minor obstacle / novel structure in
  movement space; (c) behaviour — a novel object for the remaining 4 rats (possible neophobia /
  exploration, or a new perch/rest/landmark). **Date it out once removed** (removal time TBD).

- **Barn light turned ON ~22:00 EDT 2026-07-09 and left on (forgot to switch off).** The barn is on
  the **south side, ~276 in from the paddock**. This introduces a **directional night light source on
  the south from ~22:00 07-09 onward**, and it falls **inside the night-active window (21:00–05:00)** —
  so it is a **covariate / regime boundary, not an exclusion**, that can act on **both** the animal
  path and the sensor path: (a) behaviour — a novel directional light cue / possible landmark; nocturnal
  rats may avoid or orient toward the lit south edge, biasing night movement, route structure,
  following, and shelter choice on affected nights; (b) CV — added visible light can shift CH01/CH02
  auto-exposure or partially pull them out of IR/B&W mode near the lit side, and the light itself is a
  bright spot in frame; (c) scope — affects **07-09→07-10 and later nights only**; all WISER/CV results
  through 07-08 predate it and are unaffected. **Date it out once switched off** (time TBD). South +
  ~276 in is provisional in the unverified WISER inch frame — no absolute-position claim.

- **Duo 3 encoder retuned (~evening 2026-07-09) to test the daytime keyframe cap.** The Duo 3 16MP
  main stream hard-caps every keyframe at 2,000,000 bytes, so bright-hour keyframes overflow and the
  **bottom band is never recorded** (root cause: `docs/methods/duo3_keyframe_2mb_cap.md`;
  `change_log/2026-07-09-duo3-keyframe-cap.md`). Rather than the documented fix (I-frame interval
  2×→1×), the **alternative lever** was applied: **kept I-frame Interval 2×, raised Max Bit Rate
  10240 → 12228 Kbps, switched CBR → VBR.** Hypothesis: if the 2 MB ceiling is the CBR rate-control's
  peak-frame (VBV buffer) limit rather than a fixed firmware cap, then VBR + a higher max-rate raises
  the allowed peak keyframe size and lets the full daytime keyframe through. **A covariate/regime
  boundary, not an exclusion rule:** footage before this change keeps the capped-GOP daytime bottom-
  band loss; footage after may (or may not) be clean. **Verify on 2026-07-10:** probe a bright / low-
  sun afternoon hour (worst case 16:00–18:00) of the *new* footage with `cvpipe/scripts/probe_video.py`
...
- `wiser/configs/rat_identities.csv` — animal ↔ tag mapping, Sova `valid_until`.
