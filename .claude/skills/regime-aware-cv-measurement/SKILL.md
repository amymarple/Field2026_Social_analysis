...
`glass_regime` discipline here applies to **CH05/CH06 only**). BUT they are **NOT a clear camera**: no
night vision — a **long exposure (config finalized 2026-07-08 00:49 EDT: CH07 1/6 s @ 6 Hz, CH08 1/8 s @
8 Hz, 3DNR off, gain 80)**, so only **rough visibility** (low-light noise + motion blur). Treat CH07/CH08
as a **coarse STATE / occupancy sensor** (a fog-immune *presence/state* corroboration for CH05/CH06),
**NOT** clean counting / ID / ground truth — their failure modes are **darkness / low-light /
long-exposure blur**, not the IR-glass fog regime. **Not yet calibrated / zoned / validated; capability
still needs detailed evaluation. Analyze CH07/CH08 data ONLY from after 2026-07-08 00:49 EDT (night) —
earlier data is pre-finalization, exclude.** See `configs/field_layout.json` (`sensor_class`, `settings`,
`analyze_data_from`).
