...
discipline does **not** apply to them. BUT they are **NOT a clear camera**: no night vision — a **long
exposure (finalized 2026-07-08 00:49: CH07 1/6 s @ 6 Hz, CH08 1/8 s @ 8 Hz, 3DNR off, gain 80)** → only
**rough visibility** (low-light noise + motion blur). Treat CH07/CH08 as a **coarse STATE / occupancy
sensor** (fog-immune presence/state corroboration for CH05/CH06), **NOT** clean counting / ID / ground
truth; their failure modes are **darkness / low-light / long-exposure blur**. **Analyze CH07/CH08 data
ONLY from after 2026-07-08 00:49 (night) — earlier data is pre-finalization, exclude.** They are not yet
calibrated / zoned / validated and their capability still needs evaluation — audit a CH07/CH08 output only
when one is handed to you, and treat it as a distinct, coarse camera/regime (see `field_layout.json`
`sensor_class` / `settings` / `analyze_data_from`).
...
- `cv/outputs/CH0{5,6}_sleep_<date>.csv` — 15 metric columns + (since
...
`wiser/scripts/analyze_sleep_site_cv_crossval.py` (WISER = fog-immune reference; CV
...
Write **exactly two files** and nothing else, into `cv/outputs/audit/`:
