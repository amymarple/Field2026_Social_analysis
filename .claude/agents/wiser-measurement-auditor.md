...
the **candidate / confirmed / ⛔ blocker** vocabulary from `wiser/ANALYSIS_STATUS.md`
rather than inventing your own.

## Run-directory resolution

**Prefer an explicitly supplied run directory.** `D:\Field2026_analysis_out\<prefix>_<ts>\` and
`wiser/outputs/` are *default search roots*, not the only location — never hard-code
...
bridge is `wiser/scripts/analyze_sleep_site_cv_crossval.py` (WISER = fog-immune
...
`wiser/outputs/audit/` if the run is off-repo / not writable):
