...
Usage:
    python ephys/check_offload_sizes.py --cohort 2026c --since "2026-09-03 16:00" \\
        --card SF7=15:456789.123 --card SF8=3:... --card SF9=4:...
      --card <animal>=<records>:<total MB from the console>   (repeat per card)
      --since  only session folders starting at/after this local time count as "this offload"
...
OVERHEAD_PER_RECORD = 6.5e6      # bytes the card keeps per record beyond the 65-lane stream (measured on 2026-09-02..04 checks)
...
def main() -> None:
    ...
    ap.add_argument("--since", required=True, help='local time "YYYY-MM-DD HH:MM"; sessions starting at/after it belong to this offload')
    ...
    since = datetime.strptime(a.since, "%Y-%m-%d %H:%M")
    ...
    for root in roots:
        for animal, mac, sdir in iter_raw_sessions(root, cfg.get("session_glob", "*"), cohort=a.cohort):
            ...
            if meta is None or meta["start"] < since:
                continue
            amp = sdir / "amplifier.dat"
            if not amp.exists():
                continue
            st = amp.stat(); n = st.st_size // BYTES_PER_SAMPLE_AMP
            ...
            per_animal.setdefault(norm(animal), []).append({
                "animal": norm(animal), "root": str(root), "session": sdir.name, "amp_bytes": st.st_size,
                "hours": n / float(cfg["sampling_rate_hz"]) / 3600.0,
    ...
    rows, verdicts = [], {}
    print(f"offload size check, cohort {a.cohort}, sessions starting >= {since:%Y-%m-%d %H:%M}\n")
    for an in sorted(set(per_animal) | set(cards)):
        sess = sorted(per_animal.get(an, []), key=lambda r: r["session"])
        pred = sum(r["predicted_card_bytes"] for r in sess)
        n_card, mb_card = cards.get(an, (None, None))
        print(f"== {an}: {len(sess)} folders on disk" + (f", card lists {n_card} records / {mb_card:,.3f} MB" if n_card else ", no card listing given"))
        for r in sess:
            ...
            rows.append({**r, "card_records": n_card or "", "card_total_mb": mb_card or ""})
        if not n_card:
            verdicts[an] = "no listing"
            continue
        resid = mb_card * 1e6 - pred
        beyond = resid - OVERHEAD_PER_RECORD * n_card
        missing_s = max(beyond, 0.0) / (BYTES_PER_SAMPLE_CARD * float(cfg["sampling_rate_hz"]))
        ...
        elif len(sess) > n_card or resid < -OVERHEAD_PER_RECORD:
            v = f"CHECK - disk holds MORE than the listing ({pct:.3f} %): wrong --since window or a card mix-up"
